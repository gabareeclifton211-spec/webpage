...
```bash
cd ~/mysite
cat > ~/mysite/config.local.json << 'EOF'
{
    "MASTER_PASSWORD": "Tonia@7624",
    "SECRET_KEY": "REPLACE_WITH_YOUR_RANDOM_SECRET"
}
EOF
```

This file will be ignored by git because `config.local.json` is included in `.gitignore`.

If you prefer, you can also set these values in the Web tab → Environment variables; either approach works.

WSGI tip: If you keep `config.local.json`, add this snippet at the top of your WSGI file (before importing the app) so the environment variables are available to your Flask app:

```python
import os
import json

# Path to your project (adjust username)
path = '/home/yourusername/mysite'
if path not in sys.path:
    sys.path.append(path)

# Load local secrets into environment (optional)
cfg = os.path.join(path, 'config.local.json')
if os.path.exists(cfg):
    try:
        with open(cfg) as f:
            data = json.load(f)
            if data.get('SECRET_KEY'):
                os.environ.setdefault('SECRET_KEY', data.get('SECRET_KEY'))
            if data.get('MASTER_PASSWORD'):
                os.environ.setdefault('MASTER_PASSWORD', data.get('MASTER_PASSWORD'))
    except Exception:
        pass
